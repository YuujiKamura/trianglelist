...
/*
  ...
  */
        ...
        drawTextWithLineBreak(koujiname_, xr, yb)
    ...
    private fun drawTextWithLineBreak(
        text: String,
        xr: Float,
        yb: Float
    ) {
        if (text.length > MAX_LENGTH) {
            splitAndDrawText(text, xr, yb)
        } else {
            writeText(text,
                com.example.trilib.PointXY(xr - 150.0f, yb + SINGLE_LINE_OFFSET), 1f, 7, 8f, 0)
        }
    }

    private fun splitAndDrawText(text: String, xr: Float, yb: Float) {
        ...
        writeText( splitText[0],
            com.example.trilib.PointXY(xr - 150.0f, yb + FIRST_LINE_OFFSET), 1f, 7, 8f, 0)
        if (splitText.size > 1) {
            writeText( splitText[1],
                com.example.trilib.PointXY(xr - 150.0f, yb + SECOND_LINE_OFFSET), 1f, 7, 8f, 0)
        }
    }
