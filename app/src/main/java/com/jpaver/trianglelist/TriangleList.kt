...
//        float printScale = 1f; //drawingScale;
    ...
    fun rotate(bp: PointXY, angle: Float, startnumber: Int, separationFreeMode: Boolean?) {
        ...
        // 0番からすべて回転させる場合
        if (!(startnumber > 1 && trilist_[startindex].parentBC_ >= 9) || !separationFreeMode!!) {
            this.angle += angle
            basepoint = bp.clone()
            startindex = 0
        }
    ...
    fun recoverState(bp: PointXY) {
        ...
        for (i in trilist_.indices) {
            ...
            if (trilist_[i].isPointNumberMoved == false) trilist_[i].pointNumber_ =
                trilist_[i].pointNumber_.rotate(basepoint, angle - 180)
        }
    }
    ...
    override fun addCurrent(i: Int): Int {
        selectedNumber = selectedNumber + i
    ...
    fun add(nextTriangle: Triangle, numbering: Boolean): Boolean {
        ...
        if (nextTriangle.parentNumber_ > 0) {
            val parent = getMemberByIndex(nextTriangle.parentNumber_)
    ...
    fun insertAndSlide(nextTriangle: Triangle) {
        ...
        getMemberByIndex(nextTriangle.parentNumber_).setChild(nextTriangle, nextTriangle.parentBC)
    ...
    override fun remove(number: Int) {
        ...
        //１番目以下は消せないことにする。
        if (number <= 1) return
        trilist_[number - 1].nodeTriangleA_.removeNode(trilist_[number - 1]) //removeTheirNode();
    ...
    fun resetFromParam(prms: Params): Boolean {
        ...
        val pn = prms.pn
        if (pn != tri.parentNumber_ && pn < trilist_.size && pn > 0) tri.setNode(
            trilist_[pn - 1],
            0
        )
        tri.reset(prms)
    ...
    fun traceOrNot(t: Triangle?, node: Triangle?, origin: Int, olp: ArrayList<PointXY>) {
    ...
    override fun getParams(i: Int): Params {
        if (i > trilist_.size) return Params(
            "",
            "",
            0,
            0f,
    ...
    fun getAreaC(number: Int): Float {
        if (number < 1) return 0f
        val index = number - 1
        val clr = trilist_[index].color_
        val listByColor = spritByColors()[clr]
        System.out.printf("listbycolor[%s], size %s%n", clr, listByColor.size())
        return (Math.round(listByColor.getArea() * 100.0) * 0.01).toFloat()
    }

    val info: String
        get() {
            val angle = angle.toString()
            val area = getArea().toString()
                ...
                if (i + 1 == next.parentNumber_) {
                    ...
                    if (i + 2 < trilist_.size) {
                        ...
                        if (next.parentSide == nextnext.parentSide) me.setReverseDefSide(
                            next.parentBC_,
                            true
                        ) else if (i + 2 != nextnext.parentNumber_ && next.parentBC_ != 1) me.setReverseDefSide(
                            -next.parentBC_ + 3,
                            false
