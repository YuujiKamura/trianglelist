...
/**
 * HEADER セクションを出力する。
 * app/DxfHeader.kt から移植。android.util.Log → println、BufferedWriter → Appendable。
 */
class DxfHeaderWriter(private val handles: DxfHandleGen) {

    companion object {
        const val HANDSEED_PLACEHOLDER = "%%HANDSEED%%"
    }
    ...
    fun write(
        ...
        val extMaxX = minOf(modelSpaceX, paper.width * 100)
        val extMaxY = minOf(modelSpaceY, paper.height * 100)

        println("=== DXF Header ===")
        println("extMax: $extMaxX x $extMaxY, paper: ${paper.width} x ${paper.height}")
