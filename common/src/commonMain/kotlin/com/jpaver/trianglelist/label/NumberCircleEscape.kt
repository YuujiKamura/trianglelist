...
object NumberCircleEscape {
    ...
    fun solve(
        ...
        list.forEachItemIndexed { num, shape ->
            ...
            // 番号位置を持てるのは Triangle だけ。Rectangle 等は pointNumberAnchor() が
            // 重心固定で apply も無視するので、そもそも提案を作らない。
            // 「外に出す」を許すまでは図形内に解が無くて自然に落ちていたが、外を許すと
            // 動かせない図形にも解が見つかってしまう (AutoArrangeMixedShapeTest が検出)
            if (shape !is Triangle) return@forEachItemIndexed
            if (shape.pointNumber.flag.isMovedByUser) return@forEachItemIndexed
            ...
            fun search(requireInside: Boolean): Move? {
                ...
                for (step in STEPS) {
                    ...
                    for (dir in directions) {
                        ...
                        if (!clear) continue
                        return Move(num, anchor, candidate, isFlagOut = !inside)
                    }
                }
                return null
            }

            // **図形内を全部試してから外に出す** (2026-08-28 user「もちろんスライドで賄える時は
            // それでいいわけで、面積がきつくなってから外に出すのを考える」)。
            // 内外を 1 つの loop で混ぜると、外の近い候補が内側の遠い候補より先に採られて
            // 「まだ中に置けるのに引出線が出る」ことになる ── 段を分けないと順序が守れない。
            val found = search(requireInside = true)
                ?: if (allowFlagOut) search(requireInside = false) else null
