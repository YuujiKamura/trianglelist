...
## 段B (着手 2026-06-13) — 三角形描画の frontend 化 + 混在図形リストへの布石

段A の「必要が生じた時」が来たので段B に着手。user 確定 (2026-06-13)「定石に寄せるのが正解」── 今後
**延長幅員タイプの路線図形・多様な図面形式のパラメータ生成・CAD としてのツールセット・AI 自動生成 /
自動レイアウト**に繋げたい (源泉: 2026-06-12〜13 の会話)。そのために「三角形リストを、三角形と台形
(既存 `Rectangle`) を**混在接続できるリスト**に拡張する」方向を決めた。引き算判断の撤回ではなく、
予防的だった整理に**動機が満ちた**ので前進させる、が正しい framing。

- **この commit の中身**: `writeTriangle` を `drawScene(buildTrianglePrims(tri))` に置換
  (`DrawingFileWriter.kt`)。3 辺・寸法値・旗線・番号・測点を DrawPrim 列に組む prim ヘルパー 5 本
  (`triangleLinePrims` / `dimTextPrim` / `dimFlagPrims` / `pointNumberPrims` / `sokutenPrims`) を追加し、
  同じ内容を直書きしていた旧 emit ヘルパー 5 本 (`writeTriangleLines` / `writePointNumber` /
  `writeDimFlagsFromLayout` / `writeSokutenFromLayout` / `writeTextDimension`) を削除 (2 重持ち解消)。
  `writeTriangle` は Dxf/Sfc で override されず基底 1 本なので全形式に効く (SfcWriter.kt:151 の明記どおり)。
- **バイト不変の根拠**: drawScene は同じプリミティブを同じ順序で呼ぶだけ。writer/golden スイート
  **33 passed / 0 failed** (DxfFileWriter 12・SfcWriter 2・Dxf/Sfc DimLayoutGolden 各 3・DrawingWriter 3・
  TrilistDim 2・DimensionLayoutParity 7・MainActivityScreenshotGolden 1)、common wasmJs compile green。
- **接続モデルの現状確認 (混在リストの設計前提)**: 汎用基底 `EditObject` が既に `node` (接続グラフ) +
  `initByParent` を持ち、`initByParent` は `parent is Triangle` / `parent is Rectangle` を分岐して両者を
  共有辺で繋ぐ (`EditObject.kt:27`)。`Rectangle` は length=延長 / widthA=底辺 / widthB=上辺 の台形そのもの
  (`Rectangle.kt`)。だが**本線の接続は `Triangle.nodeA / parentnumber / connectionSide`** で回り
  (`TriangleList.recoverState / resetTriangles`)、汎用 `node` は未使用。`EditList` / `RectriangleList` も
  器だけで本線は `trilist: ArrayList<Triangle>`。= user の言う「差し込むのが難しい」の正体。
- **次段の順序** (user 確定: 新しい混在リストを作り golden 全一致なら本線置換、三角形リスト保持で連結
  座標を中抜きするアダプタ案は「煩雑」で却下): (1) `writeDeduction` / `writeCalcSheet` も prim 化 →
  (2) 各図形が自分の prim を出す共通インターフェースに収束 → (3) **三角形だけで golden 全一致を確認して
  本線 (`TriangleList`) を `EditObject` 列へ置換** → (4) 台形を 2 番目の図形型として追加 + 三角形↔台形を
  切り替える新規 FAB (辺A=底辺 / 辺C=上辺 / 辺B=延長)。
