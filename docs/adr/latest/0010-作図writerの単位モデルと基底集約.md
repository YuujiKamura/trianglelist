...
- **接続モデルの現状確認 (混在リストの設計前提)**: 汎用基底 `CycleShape` が既に `node` (接続グラフ) +
  `initByParent` を持ち、`initByParent` は `parent is Triangle` / `parent is Rectangle` を分岐して両者を
  共有辺で繋ぐ (`CycleShape.kt:27`)。`Rectangle` は length=延長 / widthA=底辺 / widthB=上辺 の台形そのもの
...
- **次段の順序** (user 確定: 新しい混在リストを作り golden 全一致なら本線置換、三角形リスト保持で連結
  ...
  本線 (`TriangleList`) を `CycleShape` 列へ置換** → (4) 台形を 2 番目の図形型として追加 + 三角形↔台形を
