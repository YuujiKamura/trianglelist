...
import {
  ...
  genRectangleTritrapChain,
  genRectangleTritrapChain2,
  genMixedChain4,
  type ChainCase,
...
async function runChain(c: ChainCase): Promise<void> {
  ...
  //     当面は 混成 chain (mix4, trap-recttri-chain2) でのみ assert を走らせる。
  // 共有点 0 = 完全に別位置で「繋がってない」 = 道路測量アプリで ありえない描画。
  // 重なり時 (overlaps.length > 0) は座標が不定なので skip。
  const mustAssertConnection =
    c.label.startsWith('mix4/') || c.label.startsWith('trap-recttri-chain2/');
...
describe('chain-recttri: 親台形 BCD × 子三角形 (RectChild) chain', () => {
  const cases = [...genRectangleTritrapChain()];
...
// 親=台形 / 子=三角 / 孫=三角 の混成 chain で、 孫の sideCount=3 + 砂時計なし + 親 RectChild の位置に
// 重ならないことを assert。 RectChild タグ廃止後の Triangle 1 種統合 schema が壊れていれば孫が描画されない
// (= groupByTri に key=3 が無い)、 or 親 RectChild と同位置に重なって detectTriOverlaps が >0 を返す。
describe('chain-recttri-2: 親台形 → 子三角 → 孫三角 (混成 chain 2 段目)', () => {
  const cases = [...genRectangleTritrapChain2()];
