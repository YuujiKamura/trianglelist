...
function draw(canvas: HTMLCanvasElement, prims: Prim[]): void {
  ...
  for (const p of prims) {
    ...
    let color = isSelectedDed ? COLORS.accentOrange : isFrameText ? COLORS.frameText : (COLORS[p.layer] ?? '#000000');
    if (isFrameText && p.color !== undefined) {
      const webColorMap: Record<number, string> = {
        100: '#FF99AA', // 桃
        101: '#FFAA99', // 橙
        102: '#D0A000', // 黄 (濃いめに見やすく)
        103: '#00cc00', // 緑 (濃いめ)
        104: '#3399FF', // 空 (濃いめ)
      };
      if (webColorMap[p.color] !== undefined) {
        color = webColorMap[p.color];
      }
    }
    if (p.type === 'line') {
  ...
  if (selectedDim) {
    ...
    if (p) {
      ...
      haloText(ctx, ['A', 'B', 'C', 'D'][side] ?? '', w / 2 + pad + 4, top + fh / 2, COLORS.haloDark, COLORS.selectYellow);
      ctx.restore();
    }
  }
...
function parseCsvToState(csv: string): void {
  ...
  for (const line of csv.split(/\r?\n/)) {
    ...
    if (chunks[0] === 'Rectangle') {
      ...
      const rectExtras = chunks.slice(10);
...
function syncForm(): void {
  ...
  const newParentVal = input('newParent').value.trim();
  if (newParentVal !== lastNewParentVal) {
    lastNewParentVal = newParentVal;
    if (newParentN !== null && newParentN >= 1 && newParentN <= rows.length) {
      const pRow = rows[newParentN - 1];
      let inheritedAlign = 2; // default: 2 (右)
      if (pRow.kind === 'rectangle') {
        inheritedAlign = pRow.align ?? 0;
      } else {
        const cp = connPartsOf(pRow);
        inheritedAlign = cp ? cp.lcr : 2;
      }
      select('newLcr').value = String(inheritedAlign);
    }
  }
...
function syncFillColorFab(): void {
  ...
  const r = rows[n - 1];
  if (r) {
    const cIdx = parseInt(r.extras[4] ?? '');
    if (cIdx >= 0 && cIdx < FILL_PALETTE.length) {
      colorIndex = cIdx;
    }
  }
  b.style.background = FILL_PALETTE[colorIndex];
}
